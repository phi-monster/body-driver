...
pub fn pose_action(
    ...
    // 出现 `_ee_pose` 算 ee,**两种都出现直接 "Multiple action types found"**。所以走位这条
    // 必须两条臂都发位姿 —— 另一条臂发它**当前**的位姿,就是"待在原地"。
    let (l, r) = if arm == "right" {
        (left_pose.to_vec(), mine)
    } else {
        (mine, right_pose.to_vec())
    };
    Value::Map(vec![
        (Value::String("left_ee_pose".into()), arr(&l)),
        (Value::String("right_ee_pose".into()), arr(&r)),
        // 归一化 ⇒ 关节原始单位,理由见 `jaw_raw`。两条发动作的路都要走它,漏一条就有一半命令是错的。
        (Value::String("left_ee_joint_state".into()), arr(&[jaw_raw(left_jaw)])),
        (Value::String("right_ee_joint_state".into()), arr(&[jaw_raw(right_jaw)])),
    ])
}
