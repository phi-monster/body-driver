...
impl 自图 {
    ...
    pub fn 喂(&mut self, 前: &[u8], 后: &[u8], w: usize, h: usize, 命令: &[f64], 搜: usize) {
        ...
        for gy in 0..self.行 {
            for gx in 0..self.列 {
                ...
                // 边界上的格子搜不动,跳过(它们本来也在画面边上,见"贴边的丢掉"那条)。
                if x0 < 搜 || y0 < 搜 || x0 + self.边 + 搜 >= w || y0 + self.边 + 搜 >= h {
                    continue;
                }
                let Some((du, dv, 明)) = 块跑了(前, 后, w, x0, y0, self.边, 搜) else { continue };
                // 一片死白/死黑的格子(桌面、墙)匹配出来的位移没有意义 —— 它"跑了多少"
                // 完全由噪声决定。判据是**它自己有没有花纹**:格内灰度的极差。
                // 这不是一个填的阈值,是"这个格子有没有可以被跟踪的东西"。
                if 明 < 8 {
                    continue;
                }
                let i = gy * self.列 + gx;
    ...
    /// 这个格子是不是我。
    ///
    /// 判据只有一句:**它动过,而且动的那部分基本被"我的命令"解释掉了。**
    /// `解释率 = 1 − 残/动`,是**无量纲**的比值 —— 换相机、换分辨率、换身体都不用改。
    /// 问得太少的格子一律不算(巧合还没被磨掉)。
...
///
/// 返回的位移单位是**像素**;"有多少花纹"= 格内灰度极差,用来把一片死白的桌面挡掉
///(那种格子匹配出来的位移完全由噪声决定)。
fn 块跑了(前: &[u8], 后: &[u8], w: usize, x0: usize, y0: usize, 边: usize, 搜: usize) -> Option<(f64, f64, u8)> {
    ...
    let 明 = hi.saturating_sub(lo);
    let (mut best, mut bx, mut by) = (u64::MAX, 0i64, 0i64);
    let s = 搜 as i64;
    for dy in -s..=s {
        for dx in -s..=s {
            let mut ssd: u64 = 0;
            ...
            for y in 0..边 {
                let r0 = (y0 + y) * w + x0;
                let r1 = ((y0 as i64 + y as i64 + dy) as usize) * w + (x0 as i64 + dx) as usize;
                for x in 0..边 {
                    let d = 前[r0 + x] as i64 - 后[r1 + x] as i64;
                    ssd += (d * d) as u64;
                }
                if ssd >= best { 弃 = true; break }
            }
            if !弃 && ssd < best { best = ssd; bx = dx; by = dy; }
        }
    }
    if best == u64::MAX { return None }
    Some((bx as f64, by as f64, 明))
}
...
mod tests {
    ...
    fn 跟着我动的才算我() {
        ...
        assert!(自.是我(gx, gy, 3, 0.5), "跟着命令动的方块该被判成我(格 {gx},{gy})");
        // 远处的背景格子不该是"我"
