...
0. 🔴🔴 **炮 CJ(2026-09-03)模型一次都没被问过段** —— `BL_PILOT` 打开时我把"模型定段"那条路关了,整炮是手写老流程;而六行机器(朝向解出来)又藏在"头相机认出两个接触面"之后,那一步被 **1–3 px 的渲染散点**拒了 3/3(钳口本身是干净的 110×60 px,渲图为证)。两处都已改:下一炮**不开飞行员**,认接触面加"深度也得变"这道门。
...
**下一步(判据先写死)**:
- **炮 CK**(**不开飞行员**:模型定每一段"动第几号 → 到第几格";认接触面带深度门;两个面各去自己那一点、六行一起解、朝向解出来):**赢 = 那件东西离开桌面**(owner:*"赢的判决只有把球抓起来"*),别的都不算。先看三件:`[身] 🧠` 行数 > 0(模型真的在定段)· 认接触面过没过(过了才有六行机器)· 输的形态(两个面锁错 / 姿态解出来了但 IK 整条拒 / 又退到腕相机交接)。**每炮出视频 + 大白话分析,owner 点头才开下一炮。**
