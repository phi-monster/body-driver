...
| 项 | 绝对数 |
|---|---|
| 官方 55 集成绩 | **0 次尝试** · 目标 **≥42/55**(超人类遥操 76.03%) |
| 进入干活模式 | 2026-08-25 **首次**(此前 14 炮全 0) |
| 已量到的身体量 | **7–8 / 15**(home_pose · latency · backlash · reach · step_delivery · image_jacobian · hand_pixel · contact_threshold) |
| 量不到的 | `gripper_span`(相机够不着:指行程 1.8 cm @ 0.56 m ⇒ **9 像素**) |

**具名阻塞(按发现顺序,前两条已修待验):**
1. ~~解相机被塞在「量爪宽」那一相里当副产品~~ ⇒ 已挪到干活入口走**机体自报标定**,与爪宽解耦。
2. ~~工具轴/工具长只给 8 个循环 ⇒ 三列共 4 个弧点,估计器要不到~~ ⇒ 循环数 8→30,待验。
3. `gripper_span` 用相机量不到 ⇒ 改成**干活现场晃一下爪子当场看**,看不出来就用量到的工具长当**临时尺**并在日志明说。

**下一步:炮1(标定,bootcal)→ 炮2(记分,`general_pickup` 55 集,官方 `franka_grasp` 时序)。**
赢 = ≥42/55。输 = 低于此 —— 但**第一次有可看的失败录像**,而在此之前连失败都没有。
