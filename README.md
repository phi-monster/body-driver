...
**下一步(判据先写死)**:
- **CG 及后续**:让模型自主伸手跑完整段,赢 = `🟢 物体已经在两指中间` → `指间有东西`;输的形态要看:是"伸手"停在原地不动,还是收尾又追错东西。
- **加速第二条**:suffix decoding(`arctic-inference` 已装)单独对照 —— 我们每步同一个 JSON 骨架,是它的最佳工况。
- 之后:把 `memory.rs` 接进模型循环(阻塞:它每段只看得见上一句)。
...
🔴 **层是完整的;`plug/ws/src/main.rs` 尚未接入它**(grep 调用 0 次)—— 能被调用不等于被调用了。**接入是这一版的必做项(2026-09-03)。**
