...
**"抓握 k 去 X" 在驱动里固定等于五步,每步都是量出来的(不是策略,是这个词的含义)**:
1. **认手指**:手臂停稳,只晃 k 的抓握,深度去遮挡,画面里跟着动的那一片 = 手指区(中心、深度)。*记录:活的 —— 唯一一次抓起来(N40)就是它开的头;判死的只是"每拍都晃"(430 步/拍)和 Franka 头相机(手指只有几像素)。*
2. **摆朝向**:转手腕(转动通道,响应量的),直到手指区成为我身上**离相机最远**、且落在手腕正下方的那一片 ⇒ 沿相机视线进。能看见它就说明相机到它之间是空的;侧面相机就从侧面进。*记录:没有(旧"竖腕直下"是写死的,已删)。*
3. **对准**:方向盘航位推算把手指区中心推到 X 的像素上,**不跟模板**;推算距离超过手指区半宽就再晃一次校准;差小于半宽的一半为止。*记录:航位推算有(V5);"只晃不跟模板"没试过。*
4. **压到碰上**:沿视线往里,实到停住为止。*记录:有。*
5. **合到停住**:读数比空合高出噪声 ⇒ 指间有东西,照实报。*记录:有。*
...
|---|---|---|---|
| RoboDojo 抓球 | 抓握 k at 球(到顶住)· 球 front of 桌上某物(抬) | "抓握去 X"五步(见上)· 多手通道一起解 | **0/55**;13 炮 0 次;五步版还没跑 |
...
**下一步(判据先写死)**:
- **"抓握去 X"五步版**(先改代码、先自检、owner 点头才起炮):自检三个数 —— 晃出来的像素数 · 摆朝向那一步"手指区深度"收没收敛 · 推算位置和再晃之间差几像素。**赢 = 球离开桌面。每炮视频 + 大白话,看完才起下一炮。**
...
```
**驱动自己不决定任何动作**:用哪只手 = 模型点名的块在哪只手上;"抓握去 X" = 上面那五步(词的含义,量出来的);合 = 模型点名"抓握去某物"。
