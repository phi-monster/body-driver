...
🔴🔴 **2026-08-25 owner 判定:昨晚让整条链跑起来的两样东西都不许用。**
- **「回原位」** —— 违反 2026-08-23 死命令(`main.rs:3513`):直播删复位键,**不存在可回的家**,上一个任务停在哪就是哪。
- **问机体要相机内外参** —— 不是自标定。驱动要**全机体 zero-shot**,唯一允许的来路是自己量。
  而且相机**每一帧都在动**(机体在移动),"开机量一次"这个形状本身就不成立。

⇒ **爪宽 0.0533 / 法兰到指尖 0.1088 作废**(都经过自报相机换的单位)。

**下一步(两件,顺序固定):**
1. 把 `全相机` 自解从「量爪宽的副产品」里拆出来,做成**边干边持续解**:手在动、手在画面上的位置也在动,这些成对观测每一拍都在白送。
2. 干活侧的合爪判据删掉、引标定那一份(**无阈值**:合到读数一模一样不再变;停在 0 = 空,停在 0 以上那个数 = 东西有多宽)。
