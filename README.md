...
### 🔴 延迟是**量出来的身体常数**,不是我们要去解决的工程问题(owner 2026-09-02 定)

上层的 VLM **可以换成任何一个**,客户的算力平台**我们不知道**,甚至可能直接连云端。
⇒ **任何"把这个模型在这台服务器上跑快"的手段,都不属于这一层** —— 云厂商自己就在做,轮不到我们。

**属于这一层的是"让它慢也没关系",而且它是可量的:**

| | |
|---|---|
| 驱动能在**不问**的情况下自己跑多久 | 量出来的 |
| **该问的那一刻**是什么时候 | 预测崩了就是那一刻(方向盘本身是预测器) |
| 等回话时**不许停** | 问出去的同时手照常在动 |
| 回话没来 / 断网 | **永远正确的反射**:冻住 + 不松手 + **沿自己刚走过的路退回去**(那是世界上唯一一条有正面证据是空的路) |

🔴 **开机顺手量一次"我这个脑子回一句话要多久"**,慢(云端 2 s)⇒ 一次多要一点、更靠自己的预测扛;
快(本地小模型 0.1 s)⇒ 问得更勤。**和量"走一步实际走多远"是同一件事 —— 延迟也是量的,不是填的。**

**加速手段以【文档】随驱动走,不进代码**:见 `results/eyespeed-sep2026/`(自己部署时,我们量到的那些)。
