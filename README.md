...
### 现在具名的阻塞(2026-08-29,渲图定案)

**死因:`看爪`(认出自己的两瓣钳口)跑在头相机上,而那是一道【赢不了】的闸。**
YF 一集 89131 帧,连着 **344 次**"这一眼没看清爪子",每次挪一步再试;
建表 / 追 / 收尾 **一次都没进过**(`[收尾]` 日志 **0 行**)。

**渲图定案**(两台同一帧对比):

| | 看得见爪子 | 看得见桌上东西 |
|---|---|---|
| 头相机(第 0 台) | ❌ 胳膊只在左下角一小块,爪子不在画面里 | ✅ 全都看得见 |
| **腕相机(第 1 台,长在手上)** | ✅ **两瓣占满画面正中**(四帧全是) | ✅ 球拍/剪刀/牛仔裤都在 |

**出口不是挪,是换那台看得见的相机。** 腕相机收尾那 194 行早就写好了
(不问眼睛 —— 眼在那台里会指到机器人自己;靠"手一动、画面里动的是世界"分出物体),
但它内联在"追完之后",而追要先过这道闸 ⇒ **写好了却够不着**。
已提成具名闭包 `收尾腕`,`看爪` 失败分支每 8 次试一次(`00e7a773`)。

⚠️ **一条撤回**:此前据 `LAB "相机量爪宽在这具身体上够不着"` 断言"这具身体物理上给不出爪子信号"
—— **渲图当场证伪**。引 LAB 的一行 ≠ 在这具身体上量过。

**下一炮 YG 的判据** —— 赢:`[收尾]` 日志出现,且走到 `🟢 物体已经在两指中间`。
输:`[收尾]` 出现但物体从不进腕相机视野 ⇒ 说明还缺"把腕相机瞄向目标"这一段。
