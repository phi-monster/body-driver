...
## 现在在哪(2026-09-05,一屏读完)

🔴 **官方 `general_pickup`:0 / 55。至今没有抓起来过任何一个东西。** 目标 **≥42/55**(超人类遥操 76.03%)。
这一栏是唯一算数的成绩,不许拿中间指标替代。**2026-09-03 一天 13 炮(CJ–CU),抓起 0 次。**

### 🔴🔴 脑子 / 小脑 的分界(owner 2026-09-04 定,这是"VLM 绝对主角"的准确含义)

**人用脑子决定的事全归模型;人不假思索就做的事全归驱动。** 抓什么、用哪只手、什么时候合、什么时候算完、抬到哪 —— 脑子的事,模型说。
手怎么挪过去、腕怎么转、合到多紧 —— 小脑的事,驱动做,不问它。分界线 = 词表里每个词的含义都是**量出来的动作**,不是策略。
三天的失败两头都做反了:让模型干小脑的活(估像素、估远近 ⇒ 挑错手三炮),又让驱动干脑子的活(我写的触发器 ⇒ 删光)。
**模型的话从 CK 起每段都说得通;抓不起来全在驱动这一边** —— 主角没上过台,谈不上演砸。

**"抓握 k at X" 在驱动里的含义(2026-09-05 改;五步版撤回 —— owner:*"拧瓶盖、擦桌子、拉抽屉,无数个任务,你写无数遍去吧"*)**:
不写每个任务一套流程。只有**一张表 + 一个执行器**(记录 8-16 建的接触集:13 动词 × 五种手 390 格 366 验过,9-2 删词表时被我当陪葬删掉,现接回):
- **表**:碰 X 的哪几个点 · 从哪边进 · 碰上后 X 怎么动 · 到什么为止。前两格从 X 的**深度形状算**(`task::算一把`:掩膜 = 框内比中位深+¾高近的 + 框外一圈支撑面 → 减支撑面 → `contact_gen::candidates` → 两个接触点;**从哪边进 = 支撑面法向朝相机那侧**,桌上就是从上面、贴墙就是从墙外);后两格是模型的话(哪块去哪、到什么事件)。
- **执行器**:k 的**每一瓣**(抖抓握认出的两瓣,`Reading.halves`,各有框 `瓣位`)各去一个接触点 —— 第一段去接触点**外面**(沿法向退两个鼓起高度),第二段**进到**接触点;到了(或不再靠近)才合到读数不变。朝向是两瓣各自到点**长出来**的,驱动不指定手腕;绕两点连线那一维不可观(最小范数解保持现状,先接受)。五指手 = 五个点、吸盘 = 一个点,同一段代码。拧盖 / 拉抽屉 = 同一执行器 + 模型说"X 怎么动"。
- 几何全在**这台相机自己的深度图**里算(单位相机:焦距 = 画幅宽;点反投出去再投回同一张图,尺度在投影里抵消)。**不用解出来的相机**(记录判死)。
- 认瓣:哪只手还没认出两瓣,这一轮先抖一下(**每轮一次,不是每拍** —— 记录判死的是每拍晃 430 步)。瓣跟丢 ⇒ 下一轮再抖。
- `BL_LOOK=1`:只算接触点、只画(`抓_NNN_a.bmp`:绿=块 · 紫=瓣 · 白=接触点 · 黄空心=外面那点 · 黄线=进的方向),不动 —— 先给人看,再让它动。

**给脑子看字,不让它看图估**:每块离目标几格、在画面左半右半、哪只手够得着(发一条命令一动不动 = 够不着,测的)。
**它拿着字还选错 ⇒ 只把"选"换给更大的眼**(接口 = 换一个地址;一次任务问几次,慢无所谓)。
**记录里判死的四样一样不碰**:解相机(30 cm,8-20 整周)· 逐帧跟踪手指 · VLM 估像素(0.403)· 每拍都晃。
...
**七个目标逐条对照"说得出 / 做得出 / 验过没有"**(owner 2026-09-03:*"保证 driver 理论上能完成所有目标才能跑"*)。
话只有一种:`第几号 → 格子 / 相对某号的方位 / 别动;几条一起;到什么事件为止;快不快;别碰哪几号;哪台相机`。驱动只做:列编号 → 问 → 一起解 → 报。

| 目标 | 模型要说的话 | 驱动这边靠什么 | 验过没有 |
|---|---|---|---|
| RoboDojo 抓球 | 抓握 k at 球(到顶住)· 球 front of 桌上某物(抬) | 两瓣各到接触点、两段进、到了才合(见上)· 多手通道一起解 | **0/55**;13 炮 0 次;接触点版已装,先 `BL_LOOK` 只画 |
| LeKiwi 抓老鼠 | 同上,每段重问(目标在动);快 | 每拍重看;地点记忆 `Scope::Place`;底盘 = 速度通道(⚠️ 布局还没报过 base 路径) | 未跑 |
| 客厅收拾 30 分钟 | 换相机(camera=k)· 东西 → 某格 · 别动(拿着的) | 块跨轮对号 · 每台相机各列各的块 · 任务/地点记忆 | 未跑 |
| 五指 / 20 指扣扳机 | 握着的手指 别动 · 扳机指 at 扳机(到顶住) | 每根手指 = 一个通道(关节模式)= 部件图里一块;`Cmd::Grip` 每指各开各的 | 未跑(这台只有两指) |
| 楼梯 / 蹲捡 | 手 at 地上那东西(到碰上) | 腿 = 厂商控制器几个数 = 速度通道(宇树跑步 2.49 m/s 那条线),蹲是解出来的 | 未跑 |
| 躲拳 | 手 away 他的拳头(快)· 别碰 | `away` 方位 · 别碰框减半步子 · 快 | 未跑 |
| 格斗 | 拳 at 他脸那格(快,到碰上)· 快 + 直到脱手 = 抛/砸 | 快(步间不等停)· slip 事件 · 合/张与移动同循环 | 未跑 |

🔴 老实的边界:**"说得出、驱动有对应的量法"≠"做得成"。** 七行里六行一炮没跑过;抓球那行跑了 9 炮 0 次。
没深度的相机(无人机)世界块 = 眼给的框,只解横纵;速度通道的路径还没有任何身体报过。

**现在具名的阻塞(按证据排序)**:

1. 🔴 **驱动在头相机里认不稳自己的手指**(模板跟踪会丢、锁到别处:肘部赢了 8-28 · 两指互锁 8-27 · 模板留在背景 CO · 19 个散点当手指 CT)。**三天的主要死因。**
2. 🔴 **模型从像素里读不出几何**(挑近的手挑错三炮 CS–CU);记录 8-17 早定"只选不指"。
3. 🔴 **方向盘不稳**(探针从整个可达带起、被拒;除以接近零的实到把列撑爆;转动通道乱入;每段重探 36 步)。8-27 那套的两道闸(分歧<共识、实到分母)我重写时丢了。
4. 🔴 **没有"从哪边进"**:执行器只推像素,朝向没人管 ⇒ 爪子永远水平(接触集几何被我和手写流程一起删了)。
5. 🟡 相机解不稳(记录判死,方案里不用)· 单相机限制(头相机两指十几像素;腕相机看不到球)· 世界块碎块混入。
6. 🔴 **做法上的病反复重犯**:日志全绿而世界没发生 · 重造轮子(第 6 次:删了接触集又要接回来)· 新代码里又塞比例 · 没离线台子 · 连开炮不给视频。
...
**下一步(判据先写死)**:
- **接触点版**(已装):先 `BL_LOOK=1` 只算只画,看 `抓_NNN_a.bmp` —— 白点在球两侧、黄线从桌面朝上、紫框套在两瓣上,三样都对 ⇒ owner 点头 ⇒ 开一炮。**赢 = 球离开桌面。每炮视频 + 大白话,看完才起下一炮。**
...
```
**驱动自己不决定任何动作**:用哪只手 = 模型点名的块在哪只手上;"抓握 at X" = 两瓣各到接触点、两段进、到了才合(词的含义:几何算的 + 量出来的);合 = 模型点名"抓握去某物"。
