...
| ② | **13 个动词表** | 📝 **记着,等任务逼出来** | 方向对:动词是**语言的分类**,力学只认"碰哪些点 / 往哪推 / 物体最后什么样"。⚠️ **但删之前必须先把输出空间加宽** —— 现在只有"挪多少 + 转多少",而**擦桌子是覆盖、按压是力、裸绞是持续力+时间**,这三样现在没地方表达。先加维度(力 · 持续时间 · 直到什么条件),动词表会自己掉下来 |
| ③ | **VLM 给的像素坐标 u,v** | 📝 **好处是真的,但不是当前瓶颈** | 好处三条(量过的):让模型给坐标 **0.403≈瞎猜** vs 让它从选项里挑 **0.916≈超人类**;候选区域是图里真实结构 ⇒ **错了当场看得见**(而脑补的坐标和正确坐标长得一样);区域自带边界 ⇒ 不用猜半径(今天把桌面圈进点云的正是那个猜出来的半径)。⚠️ **但结论要改一个字:不是"删掉像素",是"换一个生产者"** —— "那个红色的杯子"总得被翻译成"这几个像素",这步删不掉,只能选谁来做。正解:分割器出候选、VLM 只负责挑。⚠️ 动手前先读 `README:403` 那条(`owlv2/gdino/Qwen2.5-VL` **当眼睛**已判死)|
...
| 诊断炮的步数上限 | 曾把 `general_pickup.py` 的 `step_lim` 从 200 抬到 2000 做诊断(备份 `/root/general_pickup.py.OFFICIAL`)。**那些数不是官方成绩**;官方那一栏必须在 200 步原配置下重跑 |
...
**现在的主路只说这几句(对任何身体、任何场景成立):**
```
眼指哪儿
  → 只动抓握通道,画面里跟着动的那一块 = 我的接触面(构造性身份)
  → 逐通道抖一下,记「这个通道动一点,两个接触面各跑多少」(每列六个数)
  → 接触集原样交出两个接触点
  → 六个方程对通道数个未知数,最小二乘解出各通道该动多少
  → 合到读数不再变 → 退回到下手前那两个面在画面上的位置 → 验东西真的被带走
```
**通道 = 观测里报出来的每一个能下命令的自由度**(关节 / 手指 / 桨 / 轮 / 舵),数量由布局给。
两指两个面、五指五个面、吸盘一个面加法向、无人机把桨当通道 —— **同一段代码**。
姿态是**解出来的**(每列六个数才约束得住朝向),不是被指定的。

### 现在具名的阻塞(2026-08-27 夜 查实并已修)

**上一个具名死因:通道表(= 最后一两厘米的方向盘)里混着物理上不可能的列。**
末端实到 0.0296 m,表里却写着"我的接触面在深度上跑了 0.2552 m"(8.6×)。
手确实到了物体上方(渲图两视角都看得见),但方向盘接错线 ⇒ 合下去是空的。
两个病因都已修:① 窗口式模板搜索会**静默**返回错位置(窗 ±33 px < 真实位移 34 px)
⇒ 改全画面搜;② 每列的起点沿用上一列的终点 ⇒ 改**来回差分**。

**同一天查实的第三处同类 bug**:退回那一段仍写死 `Cmd::Joints`,而这具机体一个关节命令都不响应
⇒ **夹住了也退不回去**,东西永远离不开原位。派发已收成一处(`迈通道`)。
