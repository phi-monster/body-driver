...
pub struct 目标 {
    ...
    /// 落到第几格(1 起;0 = 没说格子)。
    pub 格: usize,
    /// 相对方位:`""`(没说)/ `at` 贴着 / `above` 上面 / `below` 下面 / `left` / `right` / `front`(离相机更近)/ `back` / `away`(离它更远)。
...
pub fn 问段(
    ...
    if let Some(crate::json::Json::Arr(arr)) = j.get("goals") {
        for g in arr.iter().take(4) {
            ...
            let 格 = 整(g, "cell", 格数)?;
            let 相对 = 整(g, "of", 条数)?;
            // 🔴 说了地方就是要动;"别动"只在**没说任何地方**时才算数。(CK 实测 2026-09-03:模型把 keep 填成 true 同时给了格子,
            //  我让 keep 优先 ⇒ 23 段全部原地不动。)
            let 有地方 = 格 > 0 || (!方位.is_empty() && 相对 > 0);
            let 保持 = !有地方 && g.get("stay_put").and_then(|x| x.boolean()).unwrap_or(false);
            条.push(目标 { 号: 整(g, "item", 条数)?, 格, 方位, 相对, 保持 });
        }
    }
