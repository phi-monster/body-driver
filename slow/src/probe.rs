...
pub fn latency_from_beats(
    ...
    // 静止段自己要先静下来:后一半比前一半**明显**吵,才说明命令发出时身体还在余振。
    //
    // 🔴 原判据是 `后 ≤ 前`(一点不许多)。实测(2026-08-19,Franka,原始序列摆出来才看清):
    // 静止段整段都在噪声地板上(1e-5 量级,`0.00001×4, 0.00000×2`)—— 两半谁大谁小
    // 就是掷硬币,于是同一具好身体 24 拍的静止段大概率拒、6 拍的碰巧过。
    // 余振是**衰减**过程:真余振时后半只会小得多;后半反而**成倍**大,才是"没静下来"。
    // 2.0 是无量纲倍数(要求"明显",不是"一点不许多")。
    let 半 = r.len() / 2;
