  ...
  while IFS= read -r f; do
    awk -v F="$f" '
      ...
      { buf[NR]=$0 }
      {
        if (intest) next
        line=$0
        # 🔴🔴 **多行字符串要跨行跟踪。** 下面那条"数左边引号数是奇数 ⇒ 在字符串里"是
        # **逐行**算的,而多行字符串(`debt.rs` 那本账里整段散文)的中间几行**一个引号都没有**
        # ⇒ 偶数 ⇒ 被当成代码。实测:`debt.rs` 15 处全是账本散文里的数
        # ("x5 = 0.145, franka = 0.102" 这类),全部虚报。
        # ⇒ 用一个跨行的状态机:逐字符走,遇到**未转义**的引号就翻面;整行都在字符串里就跳过。
        # 这条闸是棘轮,虚报会把"还欠多少"整体抬高,让真正该改的淹在噪声里。
        {
