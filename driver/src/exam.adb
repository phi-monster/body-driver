   ...
   begin
      ...
      for I in 0 .. Natural (R.Things.Length) - 1 loop
         ...
         begin
            ...
            if T.Ratio > 1.0 then
               --  只描述【存下来的原始表】。解算那边已经把每一行按自己的一格效果归一过了,
               --  所以这个比值不等于"话语权差这么多" —— 它说明的是:一行响一行哑,哑的那一行
               --  一格才推动这么一点点,靠它做判据本来就更容易被噪声淹掉。
               Put_Line ("     活着的行里,最响的一格推动是最哑的 " & Codec.Fmt (T.Ratio, 0) & " 倍"
                 & "(解算已按各行自己的一格效果归一,所以这不是话语权之差;它说明最哑那行本身就细微,判据要更小心)");
            end if;
         end;
      end loop;
      Put_Line ("");
