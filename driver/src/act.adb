      ...
      case P.Kind is
         when Piece_Pt =>
            ...
            begin
               ...
               --  🔴 只有【这台相机没有深度】的时候才需要用光流去估"这只手看着多大"。
               --  有深度就别估:这个估计是个正反馈(采到的光流一噪,间距就往小里走),
               --  实测会从 0.18 塌到 0.009,把"大小"那一行带飞,整段跑散(IU 实测:差距 1 → 40)。
               if P.Chan_K = Chan.Per_Arm and then not F.Cams (Cam).Has_Depth then
               ...
               for R in 0 .. Table.Rows - 1 loop
                  ...
                  begin
                     ...
                     if Per_Step > 0.0 then
                        ...
                        --  🔴 一行最多只能要求【这一段允许的步数】那么多。不压这一条,一行算出
                        --  "还差一千步"就会把整个目标吃掉,手开始乱转(IF 实测:大小 4 → 1342)。
                        --  ⚠️ 压成"各要一推"是过头了:各行的轻重全没了,解算只会给一个互相抵消的小折中,
                        --  手一段 60 推爬不动(IN 实测:三行各要 1 推,命令只有一推的八分之一)。
                        --  用"这一段的步数"当上限:本来就走不了比这更多,既挡住爆表,又保住轻重。
                        declare
                           Cap_Steps : constant Long_Float := Long_Float (Natural'Max (1, Step_Limit));
                        begin
                           T.Err (R) := Long_Float'Max (-Cap_Steps, Long_Float'Min (Cap_Steps, T.Err (R)));
                        end;
