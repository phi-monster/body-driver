...
package body Act is
   ...
   procedure Geo_Approach (L : in out Plug.Link; C : in out Context; F : in out Plug.Frame; Cam, Arm : Natural; Slot : Integer;
      ...
      procedure Descend (Ev : out Unbounded_String) is
         ...
         --  ⇒ 记住"上次对它直下了多深、抬时溜走了",下一次再贴近它就少下半成张口(比例,无量纲),夹在略高处
         --  GC28:溜走后少下约 4 mm 合 0.578、抬 45 mm 不掉;GC34:少下 13.5 mm 合空 ⇒ 少下改成半成张口(比例,无量纲)
         Cut : constant Long_Float := (if C.Geo_Slid_Slot = Slot and then C.Geo_Last_Down > 0.0 then 0.05 * G.Gap else 0.0);
         ...
         procedure Centering_Offset (Mv : out Geom.V3; Ok : out Boolean; Why : out Unbounded_String;
                                     Pu, Pv, Au_Out, Av_Out : out Long_Float) is
            Cur : constant Plug.Arm_Pose := F.EE (Arm);
            Rc : constant Geom.M3 := Geom.Cam_R (G, Cur);
            Tip_W : constant Geom.V3 := Geom.Ap (Rc, G.Tip);
      ...
      begin
         ...
         --  GC33:横挪后直下到顶住再量 —— 顶住时指尖都在球心高度,是手指压到它肩上把它推走,球被两指来回踢(−41/+28/−44/+35)。
         --  ⇒ 对中全部在悬停高度做(不下去碰它):量 → 横挪 → 再量,最多五轮(次数),差不到张口的百分之五算对准;对准了才直下一次
         declare
            Gain : Long_Float := C.Geo_Center_Gain;   --  上次量到的响应比先用上
         ...
         begin
            for Round in 1 .. 5 loop
               ...
               begin
                  ...
                  if not Ok then
                     Geo_Say ("对中:" & To_String (Why) & " ⇒ 不挪,照读数直下");
                     exit;
                  end if;
                  ...
                  if Mag > 1.0 * G.Gap then   --  超过一个张口(比例,无量纲)才不像是它
                     Geo_Say ("对中:差 " & Mm (Mag) & ",超过一个张口,不像是它 ⇒ 不挪");
                     exit;
                  end if;
                  Geo_Say ("对中 第" & Codec.Img (Round) & " 轮(悬停高度):横挪 (" & Mm (Mv (0)) & "," & Mm (Mv (1)) & ")");
         ...
         --  GC36:顶住后往两边各错开 9 mm 再直下 —— 指尖实际高度没变(仍在它最宽处一层),偏 9 mm 那侧合爪反而合空。撤回。
         if Blocked then
   ...
   procedure Geo_Held (L : in out Plug.Link; C : in out Context; F : in out Plug.Frame; Arm, Cam : Natural; Slot : Integer;
      ...
      --  🔴 手指读数就是力:命令合到底、读数停在空手值以上 = 捏着东西。GC10 布局 0:手指在球腰以下合上,球被挤着往上爬,
      --  读数一路往小走 —— 那不是滑出去,是它往手心里钻;所以"读数往小走"不能当滑。能当滑的只有【读数掉到空手值附近】
      --  (指缝里没东西了,门槛 = 量程的百分之五,比例,无量纲)或【画面里它不见了】;拿住 = 读数在空手值以上 且 画面里它还在
      Slip_Floor : constant Long_Float := 0.05;
      R0 : Long_Float := 0.0;
      R1 : Long_Float := 0.0;
   begin
      ...
      Geo_Say ("抬完 " & Mm (Lift) & " 手指读数 " & Codec.Fmt (R0, 3) & " → " & Codec.Fmt (R1, 3) & "(在空手值以上 = 指缝里还有东西;往小走 = 它往手心里钻了)");
      if not Seen0 then
   ...
   begin
      ...
      declare
         --  和判拿住同一条力的规矩:读数掉到空手值附近(量程的百分之五,比例,无量纲)= 指缝里没东西了;往小走本身不算滑
         Slip_Floor : constant Long_Float := 0.05;
      ...
      for K in 1 .. Legs loop
         ...
         Up := Up + Leg;
         R1 := Selfmap.Jaw_Of (F, Arm);
         if C.Wld.Holding and then R1 < Slip_Floor then
            Event := S ("slip: while lifting (after " & Mm (Up) & ") my grip closed to its empty reading (" & Codec.Fmt (R0, 3) & " → " & Codec.Fmt (R1, 3)
