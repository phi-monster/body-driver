...
package body Act is
   ...
   begin
      --  🔴 没深度时【不能直接返回空】—— 官方观测就是 3 路 RGB + 关节 + 末端位姿,没有深度通道。
      --  原来这一句一短路,THINGS OUT IN THE WORLD 整节是空的:脑只看得见自己的手指和爪心,
      --  世界里一件东西都没有,连要抓的东西的名字都点不出来(合并后首炮 M1 实测)。
      --  ⇒ 没深度就跳过深度那几档尺子,直接走下面【按颜色切】那一路(Cut_Colour 本来就在,
      --  只是被这一句挡在后面)。有深度时行为一个字不变。
