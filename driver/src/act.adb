               ...
               for R in 0 .. Table.Rows - 1 loop
                  ...
                  begin
                     ...
                     if Per_Step > 0.0 then
                        ...
                        --  ⚠️ 这里曾经加过两条闸:"一行最多要求这一段的步数"和"差不到一推就不管"。
                        --  它们是为【没有深度】那阵子临时搭的 —— 当时某一行的分母没量到,"还差几步"
                        --  会算成上千,把整个目标吃掉(IF:大小 4 → 1342)。
                        --  🔴 但它们本身更害:每一步都有某一行刚好跨过门槛被开掉或关掉,解算的方向跟着翻,
                        --  整段看上去就是抽搐(owner 看 JA 视频:"机械臂全程在发癫",FO 从来没有过)。
                        --  病根已经在源头治好(手的远近现在是从深度图上真读的,不再是姿态表里猜的)⇒ 拆掉脚手架。
                     else
